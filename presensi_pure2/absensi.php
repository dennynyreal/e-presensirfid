...
<head>
    ...
    <style>
        ...
        .filter-container select {
            width: 160px; /* Menyesuaikan lebar dropdown */
        }
        ...
        <div class="filter-container">
            <form method="GET" action="">
                ...
                <select id="kelas" name="kelas">
                    ...
                    <?php
                        include "koneksi.php";
                        $kelasQuery = "SELECT id, kelas FROM kelas";
                        $kelasResult = mysqli_query($konek, $kelasQuery);

                        while ($kelas = mysqli_fetch_assoc($kelasResult)) {
                            $selected = isset($_GET['kelas']) && $_GET['kelas'] == $kelas['id'] ? 'selected' : '';
                            echo "<option value=\"" . htmlspecialchars($kelas['id'], ENT_QUOTES, 'UTF-8') . "\" $selected>" . htmlspecialchars($kelas['kelas'], ENT_QUOTES, 'UTF-8') . "</option>";
                        }
                    ?>
                </select>
                ...
                <tbody>
                    <?php 
                        ...
                        if (!empty($kelasFilter)) {
                            $kelasFilter = mysqli_real_escape_string($konek, $kelasFilter);
                            $query .= " AND b.kelas = '$kelasFilter'";
                        }
